...
class Edrone():
	...
	def __init__(self):
		...
		self.Kp = [1110*0.06,895*0.06,1520*0.06]
		self.Ki = [3*0.001,3*0.001,12*0.001]
		self.Kd = [2040*0.3,1512*0.3,2402*0.3]


		#-----------------------Add other required variables for pid here ----------------------------------------------
		self.prev_error = [2,2,20]
		...
		self.command_pub_alt = rospy.Publisher('/alt_error', Float64, queue_size=1)
		self.command_pub_pitch = rospy.Publisher('/pitch_error', Float64, queue_size=1)
		self.command_pub_roll = rospy.Publisher('/roll_error', Float64, queue_size=1)
	...
	def altitude_set_pid(self,alt):
		...
		self.Ki[2] = alt.Ki * 0.001# Previously it was 0.008
	...
	#	8. Add error_sum
	# Notes : it is crashing above 1500 pitch 
		...
		#Publish Here
		self.command_pub_alt.publish(self.error[2])
		self.command_pub_pitch.publish(self.error[1])
		self.command_pub_roll.publish(self.error[0])
		self.current_time = time.time()
		...
		if (delta_time >= self.sample_time):
			...
			self.i_error[0] += self.error[0] * delta_time
			self.i_error[1] += self.error[1] * delta_time
			self.i_error[2] += self.error[2] * delta_time
			self.d_error = [0,0,0]

			if delta_time > 0:
				self.d_error[0] = self.delta_error[0] / delta_time
				self.d_error[1] = self.delta_error[1] / delta_time
				self.d_error[2] = self.delta_error[2] / delta_time
			self.last_time = self.current_time
			self.prev_error[0] = self.error[0]
			self.prev_error[1] = self.error[1]
			self.prev_error[2] = self.error[2]

			self.out_roll = self.p_error[0] + (self.Ki[0] * self.i_error[0]) + (self.Kd[0] * self.d_error[0])
			self.out_pitch = self.p_error[1] + (self.Ki[1] * self.i_error[1]) + (self.Kd[1] * self.d_error[1])
			self.out_alt = self.p_error[2] + (self.Ki[2] * self.i_error[2]) + (self.Kd[2] * self.d_error[2])
...
if __name__ == '__main__':
	...
	e_drone = Edrone()
	r = rospy.Rate(55) #specify rate in Hz based upon your desired PID sampling time, i.e. if desired sample time is 33ms specify rate as 30Hz
